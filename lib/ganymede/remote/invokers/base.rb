    ...
    module Invokers
      class Base
        ...
        def initialize(routing, client_class)
          ...
          @sleep_time = Ganymede.config[:sleep_time] || 2
          @reset_time = Ganymede.config[:reset_time] || 30
          @@mutex ||= Mutex.new
        end
        ...
        # returns the Ganymede::Base.logger
        def logger; Ganymede::Base.logger; end
        ...
        protected
          ...
          #
          def routes
            @active_routes ||= Ganymede::Discovery.discovered.map { |clazz| @routing.queue_names_routing_class(clazz) }.flatten
          end
