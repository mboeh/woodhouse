...
module Ganymede
  module Remote
    module Invokers
      class ThreadedPoller < Ganymede::Remote::Invokers::Base
        ...
        def initialize(routing, client_class)
          ...
          ThreadedPoller.sleep_time = Ganymede.config[:sleep_time] || 2
          ThreadedPoller.reset_time = Ganymede.config[:reset_time] || 30
        ...
        def listen                
          ...
          Ganymede::Discovery.discovered.each do |clazz|
        ...
        def clazz_listen(clazz)
          ...
          if Ganymede.config.has_key?(:listeners)
            if Ganymede.config[:listeners].has_key?(clazz.to_s)
              config = Ganymede.config[:listeners][clazz.to_s].symbolize_keys
          ...
          while (!Thread.current[:shutdown]) do
            begin
              ...
              rescue Ganymede::GanymedeConnectionError
