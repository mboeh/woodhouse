require 'bunny'
require 'ganymede/worker_process'

class Ganymede::BunnyWorkerProcess < Ganymede::WorkerProcess
  include Celluloid
  ...
  def make_job(message)
    Ganymede::Job.new(@worker.worker_class_name, @worker.job_method) do |job|
