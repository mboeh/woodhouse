...
module Woodhouse
  module Remote
    ...
    # set the desired runner here. this is initialized with Woodhouse.default_runner. 
    ...
    @@invoker ||= Woodhouse::Remote::Invokers::ThreadedPoller
    
    # retrieve the dispatcher or instantiate it using the defaults
    def self.dispatcher
      @@dispatcher ||= Woodhouse.default_runner
    end
    ...
    def self.run(clazz, method, options = {})
      uid = Woodhouse::Remote.generate_uid(clazz, method)
      options[:uid] = uid if options.kind_of?(Hash) && !options[:uid]
      Woodhouse.find(clazz, method) # this line raises a WoodhouseError if the method does not exist. 
      Woodhouse.log_job uid, "dispatched", options
