    ...
    module Invokers
      class Base
        ...
        def initialize(routing, client_class)
          ...
          @sleep_time = Woodhouse.config[:sleep_time] || 2
          @reset_time = Woodhouse.config[:reset_time] || 30
          @@mutex ||= Mutex.new
        end
        ...
        # returns the Woodhouse::Base.logger
        def logger; Woodhouse::Base.logger; end
        ...
        protected
          ...
          #
          def routes
            @active_routes ||= Woodhouse::Discovery.discovered.map { |clazz| @routing.queue_names_routing_class(clazz) }.flatten
          end
