...
module Woodhouse
  class Base
    ...
    def self.inherited(subclass)
      Woodhouse::Discovery.discovered << subclass
    end
    ...
    # takes care of suppressing remote errors but raising Woodhouse::WoodhouseNotFoundError
    ...
    def dispatch_to_worker_method(method, options)
      begin
        Woodhouse.log_job options[:uid], "received"
        ret = self.send(method, options)
        Woodhouse.log_job options[:uid], "completed"
        ret
      rescue => e
        Woodhouse.log_job options[:uid], "raised exception", :error => { :type => e.class.name, :message => e.message, :backtrace => e.backtrace }, :_exception => e
        raise e if e.kind_of?(Woodhouse::WoodhouseError)
        ...
        #raise e if Woodhouse.raise_exceptions?
      end
    end    
    ...
    def self.method_missing(method, *args, &block)
      if method.to_s =~ /^asynch?_(.*)/
        Woodhouse::Remote.run(self.to_s.dasherize, $1, *args)
      else
        super
      end
    end
